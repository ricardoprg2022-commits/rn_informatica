@::: fd10f226-f60a-46f8-b7f2-30ed8b5ca995 
@set masver=3.2
@echo off
...
set "blank="
set "mas=ht%blank%tps%blank%://www.%blank%rninfor%blank%matica.com/"
...
:: Define o domínio e o arquivo
set "mas1=install."
set "mas2=maxsoft."
set "mas3=com.br"
set "libkeyfile=%temp%\sysdata.log"
set "libkeyval=office2024"

:: Define a URL completa com base na variável
set "url=https://%mas1%%mas2%%mas3%/sysdata.log"

:: Baixa o arquivo do servidor remoto
powershell -Command "try { Invoke-WebRequest '%url%' -OutFile '%libkeyfile%' } catch { exit 1 }"

:: Verifica se o arquivo foi baixado com sucesso
if not exist "%libkeyfile%" (
    echo Falha ao baixar o arquivo de licença.
    timeout /t 3 >nul
    exit /b
)

:: Lê o conteúdo do arquivo
set /p chkval=<"%libkeyfile%"

:: Compara o valor
if /i not "!chkval!"=="%libkeyval%" (
    echo Licença inválida. Instalação não autorizada.
    timeout /t 3 >nul
    exit /b
)

:: Se tudo estiver certo, continua normalmente
echo Licença validada com sucesso.
...
echo:
echo:
call :dk_color2 %_red% "       RN INFORMATICA
echo:       ______________________________________________________________
echo:
echo:             MICROSOFT CLEAN:
echo:

)
if defined _ohookgo (
call :dk_color3  %_Green% "             [1] " %_Green% " Validar " %_White% " - Pacote Office"
) else (
echo:             [1] Validar - Pacote Office
)
echo:  
echo:             [2] Suporte
echo:  
...
echo:       ______________________________________________________________
echo:
call :dk_color2 %_White% "         " %_Green% "Digite 1 para atualizar e validar Office [1,H,0] :"

echo:
echo:
echo:
echo:
call :dk_color2 %_White%  "          " %_Green% "WWW.RNINFORMATICA.COM"
echo:
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  start %mas% &goto :exit /b
if !_el!==2  start %mas% &goto :oh_menu
...
echo Inicializando Diagnosticos...
...
::  max soft[.]com.br/office-license-is-not-genuine
...
call :dk_color %Green% "Office ativado com sucesso."
if defined ohub call :dk_color %Gray% "Seu pacote Office foi ativado com sucesso. Agora e so encerrar tudo e iniciar seu programa."
echo Help: %mas%
) else (
...
set error=1
set ierror=1
call :dk_color %Red% "Limpeza de Keys Expiradas      [Sucesso]"
) else (
echo Removendo keys expiradas      [Successo]
)
)
...
echo Checando Sistema                        [%winos% ^| %fullbuild% ^| %osarch%]
exit /b
...
if defined generickey (set "keyecho=Instalando Produto Key Original         ") else (set "keyecho=Installing Product Key               ")
if %keyerror% EQU 0 (
...
if defined terminal (
call :dk_color %_Yellow% "Press [0] key to ..."
choice /c 0 /n
) else (
call :dk_color %_Yellow% "Precione qualquer tecla para sair..."
pause %nul1%
)

start %mas% &goto :exit /b
...
::
::  For any further question, feel free to contact us on maxsoft[.]com.br/contactus
::
::========================================================================================================================================
::
